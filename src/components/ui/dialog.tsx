...
/**
 * Enhanced Dialog component with automatic back button handling
 *
 * This component automatically registers/unregisters itself when opened/closed
 *
 * Just use: <Dialog open={isOpen} onOpenChange={setIsOpen}>
 */
const DialogRoot = ({
  open,
  onOpenChange,
  ...
  React.useEffect(() => {
    ...
    if (open && onOpenChangeRef.current) {
      // Register this dialog when it opens
      registerDrawer(dialogId, () => {
  ...
  <DialogPortal>
    ...
    >
      ...
      <DialogPrimitive.Close
        ...
        style={{
          top: "calc(var(--tg-content-safe-area-inset-top, 10px) + var(--tg-safe-area-inset-top, 0px) + 10px)",
        }}
      >
        <X className="h-4 w-4" />
